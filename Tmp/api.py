    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                set_clause = ", ".join(
                    [f"{col} = %s" for col in request.update_values.keys()]
                )
                update_query = (
                    f"UPDATE {table_name} SET {set_clause} WHERE {request.condition}"
                )
                query_params = tuple(request.update_values.values()) + tuple(
                    request.condition_params
                )
